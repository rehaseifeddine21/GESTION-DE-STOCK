...
class StockApp(MDApp):
    ...
    def fetch_dashboard_stats(self):
        url = f'http://{self.active_server_ip}:{DEFAULT_PORT}/api/daily_stats'
        body = json.dumps({'username': self.current_user_name, 'is_seller': self.is_seller_mode})
    ...
    def update_dashboard_layout(self):
        ...
        self.lbl_dashboard_date = MDLabel(text=f'Tableau de Bord ({today_str})', font_style='H6', bold=True, theme_text_color='Primary', valign='center')
        ...
        try:
            ...
            def delete_product_action(x):
                ...
                def confirm_delete(y):
                    ...
                    UrlRequest(f'http://{self.active_server_ip}:{DEFAULT_PORT}/api/delete_product', req_body=json.dumps(payload), req_headers={'Content-Type': 'application/json'}, method='POST', on_success=lambda r, s: [self.dialog.dismiss(), self.fetch_products(), self.notify('Produit supprimé', 'success')], on_failure=lambda r, e: self.notify('Erreur suppression', 'error'))
